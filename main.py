...
async def handle_error(context_message, exception, stringtrace="---empty string trace---"):
...
async def run_script():
    async with async_playwright() as p:
        ...
        while True:
            ...
            if not await check_class_status(page, context):
            ...
            #     break
            # include some randomness to avoid detection

            # weird randomness
            await asyncio.sleep(randint(3, 6) + randint(randint(-2, 1), randint(1, 3)))
...
async def duo_auth_hopt(page, context) -> bool:
    ...
    try:
        ...
        hotp = pyotp.HOTP(HOPT_KEY)

        # Referencing nonexistent variable here apparently for some reason so
        # this is a quick fix
        HOPT_COUNTER = int(os.environ['HOPT_COUNTER'])

        hotp_code = hotp.at(HOPT_COUNTER)
        TRACE += f"Generated HOTP code: {hotp_code}, at counter= {HOPT_COUNTER}\n"
        ...
        HOPT_COUNTER = HOPT_COUNTER+1
        update_env_variable("HOPT_COUNTER", HOPT_COUNTER)
        ...
        if login_button:
            try:
                ...
                update_env_variable("HOPT_COUNTER", HOPT_COUNTER-1)
        ...
        await duo_frame.page.screenshot(path='./screenshots/duo-hopt-auth-9.png')

        # need to implement checking the website to see if its still CAS
        if "Duo" in await page.title():
            raise Exception("We authenticsted but somehow did not")
...
if __name__ == "__main__":
    # Step 1: Start the FastAPI health server in a separate thread
    health_thread = threading.Thread(target=start_health_server, daemon=True)
    health_thread.start()

    # Step 2: Run the main application logic (assumed to be asynchronous)
    asyncio.run(run_script())

# This stuff below was for WebAuthn, but it's not needed since we're now using HOTP
...
#         'credential': {
#             'credentialId': credentialIdPadded,
